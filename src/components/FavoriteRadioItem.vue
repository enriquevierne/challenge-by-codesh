...
<template>
    <li class="flex items-center justify-between w-full rounded-lg bg-zinc-800">
        <div class="flex items-center gap-6 p-4 px-6">
            <div class="flex items-center p-4 rounded-full bg-zinc-950">
                <button
                    type="button"
                    v-if="musicStore.selectedMusic != station.name"
                    @click="toggleMusic(station)">
                    <PlayIcon class="stroke-0 size-14 fill-blue-700" />
                </button>
                <button type="button" v-else @click="toggleMusic(station)">
                    <SquareIcon class="stroke-0 size-14 fill-blue-700" />
                </button>
            </div>
            <div v-if="!editMode">
                <p class="font-bold md:text-3xl">{{ station.name }}</p>
                <p class="font-semibold md:md:text-xl text-zinc-400">
                    {{ station.country }}, {{ station.state }}
                </p>
            </div>
            <div v-else class="grid grid-cols-3 text-blue-500">
                <span>Edit Form</span>
                <div
        ...
        <div class="flex flex-col items-center gap-6 p-4 md:flex-row">
            <button type="button">
                <PencilIcon
                    ...
                    "
                    class="size-8 fill-zinc-950 stroke-zinc-900" />
                <CheckIcon
                    @click="updateStation(station.changeuuid, formEdit)"
                    v-else
                    class="size-8 text-zinc-950 stroke-zinc-900" />
            </button>
            <button type="button">
                <TrashIcon
                    v-if="!editMode"
